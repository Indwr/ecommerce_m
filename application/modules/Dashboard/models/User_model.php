...
//        echo $this->db->last_query();
    ...
    public function get_pool_upline($sponser_id){
        $this->db->select('downline_id,down_id');
        $this->db->from('tbl_pool_downline');
        $this->db->where(['user_id' => $sponser_id , 'down_id <' => 7]);
    ...
    public function get_pool_users($user_id){
        $this->db->select('tbl_users.user_id , tbl_users.package_amount , tbl_users.paid_status');
        $this->db->from('tbl_pool');
        $this->db->join('tbl_users', 'tbl_users.user_id = tbl_pool.user_id');
        $this->db->where(array('tbl_pool.upline_id' => $user_id ));
        $query = $this->db->get();
        $res = $query->result_array();
        return $res;
    }
