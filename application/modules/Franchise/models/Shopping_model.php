...
//        echo $this->db->last_query();
    ...
    public function get_product($user_id) {
        $this->db->select('tbl_franchise_products.product_id,tbl_franchise_products.quantity,tbl_products.id,tbl_products.title,tbl_products.mrp,tbl_products.bv,tbl_products.dp,ifnull(tbl_product_images.image,"no_image.png") as image');
        $this->db->from('tbl_franchise_products');
        $this->db->join('tbl_products', 'tbl_franchise_products.product_id = tbl_products.id');
        $this->db->join('tbl_product_images', 'tbl_product_images.product_id = tbl_franchise_products.product_id', 'left');
        $this->db->where('user_id',$user_id);
        $query = $this->db->get();
        $res = $query->result_array();
        // echo $this->db->last_query();
