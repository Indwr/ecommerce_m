...
<div class="content-wrapper" style="min-height: 378px; background:white">
    ...
    <div class="content">
        <div class="container-fluid">
            <?php echo form_open(base_url('Franchise/Shopping/place_order'),array('id' => 'ShoppingForm'));?>
            <div class="row">
            ...
            <div class="row" id="PymntRow">
                ...
                <!-- <div class="col-md-3">
                    Available Coupans : <?php //echo count($discount_coupans);?><br>
                    <select class="form-control" name="coupans">
                        <?php
                            // $coupan_limit = $total_price_with_gst / 350;
                            // if(count($discount_coupans) < $coupan_limit){
                            //     $coupan_limit = count($discount_coupans);
                            // }
                            // for($i = 0 ; $i <= $coupan_limit ; $i++)
                            //     echo'<option value="'.$i.'">'.$i.' X (Rs. 50)</option>';
                        ?>
                    </select>
                </div> -->
                <div class="col-md-3">
            ...
            <?php echo form_close();?>
            <?php echo form_open(base_url('Franchise/Shopping/AddUserAddress/') , array('method' => 'post' , 'id' => 'AddressForm'));?>
...
function get_address_list(){
    var url = '<?php echo base_url("Franchise/Shopping/getUserAddress/".$user["user_id"]);?>';
...
$(document).on('click','.dltadrs',function(){
    ...
    if (r == true) {
        var url = '<?php echo base_url("Franchise/Shopping/DeleteUserAddress/");?>'+address_id;
        $.get(url,function(res){
            alert(res.message)
            if(res.success == 1)
                get_address_list()
...
$(document).on('submit','#ShoppingForm',function(e){
    ...
    $.post(url,formData,function(res){
        ...
        t.append('<input type="hidden" name="'+res.token_name+'" value="'+res.token_value+'">')
        if(res.success == 1){
            window.location.href='<?php echo base_url("Franchise/Shopping")?>'
        }
