...
class User extends CI_Controller {
    ...
    //////////-----------------Franchise Functions Above --------------------------

    public function Referral() {
        if (is_logged_in()) {
            $response['user'] = $this->User_model->get_single_record('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), '*');
            $response['today_income'] = $this->User_model->get_single_record('tbl_income_wallet', 'user_id = '.$this->session->userdata['franchise_id'].' and date(created_at) = date(now()', 'ifnull(sum(amount),0) as today_income');
            $response['task_income'] = $this->User_model->get_single_record('tbl_income_wallet', 'user_id = '.$this->session->userdata['franchise_id'].' and type = "task_income"', 'ifnull(sum(amount),0) as task_income');
            $response['direct_income'] = $this->User_model->get_single_record('tbl_income_wallet', 'user_id = '.$this->session->userdata['franchise_id'].' and type = "direct_income"', 'ifnull(sum(amount),0) as direct_income');
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                //if($this->input->post('password') == $this->input->post('vpassword')){
                    ...
                    $checkPan = $this->User_model->get_single_record('tbl_bank_details', array('pan' => $this->input->post('pan')),'count(id) as checkPan');
                    $sponser = $this->User_model->get_single_record('tbl_users', array('user_id' => $sponser_id,'paid_status >' => 0), '*');
                    if(!empty($sponser)){
                        if($checkPhone['checkPhone'] < 3){
                            if($checkPan['checkPan'] < 3){
                                if(!empty($this->input->post('position'))){
                                    ...
                                    $res = $this->User_model->add('tbl_bank_details',array('user_id' => $userData['user_id'],'pan' => $this->input->post('pan')));
                                    if ($res) {
                                ...
                                $this->session->set_flashdata('error', 'This PAN Number is already used,Please try another one');
                                $this->load->view('register', $response);
                            }
                        }else{
    ...
    public function welcomeLetter(){
        if(is_logged_in()){
            $response['userData'] = $this->User_model->get_single_record('tbl_users',['user_id' => $this->session->userdata['franchise_id']],'*');
    ...
    public function Profile() {
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $response['success'] = 0;
                $data = $this->security->xss_clean($this->input->post());
                    $userDetail = $this->User_model->get_single_record('tbl_users',['user_id' => $this->session->userdata['franchise_id']],'city,phone,email,address,postal_code');
                    ...
                    // pr($Userdata);
                    // $Userdata['postal_code'] = $data['postal_code'];
                    $updres = $this->User_model->update('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), $Userdata);
                    if ($updres == true) {
            ...
            $response['user_bank'] = (object) $this->User_model->get_single_record('tbl_bank_details', array('user_id' => $this->session->userdata['franchise_id']), '*');
...
//            pr($response);
    ...
    public function password_reset(){
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $user = $this->User_model->get_single_record('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), 'id,user_id,password');
                if ($npassword != $vpassword) {
                    ...
                    $updres = $this->User_model->update('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), array('password' => $vpassword));
                    if ($updres == true) {
    ...
    public function trans_password(){
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $npassword = $data['npassword'];
                $vpassword = $data['vpassword'];
                $user = $this->User_model->get_single_record('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), 'id,user_id,master_key');
                if ($npassword != $vpassword) {
                    $response['message'] = 'Verify Password Does Not Match';
                } elseif ($cpassword != $user['master_key']) {
                    $response['message'] = 'Wrong Current Password';
                } else {
                    $updres = $this->User_model->update('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), array('master_key' => $vpassword));
                    if ($updres == true) {
                        $response['message'] = 'Password Updated Successfully';
                        $response['success'] = 1;
                    } else {
        ...
        if (is_logged_in()) {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                // } else {
                    ...
                    $updres = $this->User_model->update('tbl_bank_details', array('user_id' => $this->session->userdata['franchise_id']), $userData);
                    if ($updres == true) {
    ...
    public function PlaceParticipants() {
        if (is_logged_in()) {
            ...
            $response['users'] = $this->User_model->get_records('tbl_users', array('sponser_id' => $this->session->userdata['franchise_id'], 'is_placed' => 0), 'id,user_id,sponser_id,role,name,email,phone,upline_id,created_at');
    ...
    public function Directs() {
        if (is_logged_in()) {
            ...
            $response['users'] = $this->User_model->get_records('tbl_users', array('sponser_id' => $this->session->userdata['franchise_id']), 'id,user_id,sponser_id,role,name,position,last_name,email,paid_status,phone,upline_id,created_at');
    ...
    public function Downline($position = '') {
        if (is_logged_in()) {
            $response = array();
            $where['user_id'] = $this->session->userdata['franchise_id'];
            if($position != ''){
    ...
    //         $response['users'] = $this->User_model->get_records('tbl_user_positions', array('sponser_id' => $this->session->userdata['franchise_id'], 'position' => 'R'), 'id,user_id,sponser_id,upline_id,position');
    //         foreach ($response['users'] as $key => $user) {
    //             $response['users'][$key]['user'] = $this->User_model->get_single_record('tbl_users', array('user_id' => $user['user_id']), 'id,user_id,sponser_id,role,first_name,last_name,email,phone,upline_id,created_at');
    //         }
    //         $this->load->view('directs', $response);
    ...
    public function income($type) {
        if (is_logged_in()) {
            ...
            $response['total_income'] = $this->User_model->get_single_record('tbl_income_wallet', array('user_id' => $this->session->userdata['franchise_id'], 'type' => $type), 'ifnull(sum(amount),0) as total_income');
            $response['user_incomes'] = $this->User_model->get_records('tbl_income_wallet', array('user_id' => $this->session->userdata['franchise_id'], 'type' => $type), 'id,user_id,amount,type,description,created_at');
    ...
    public function Magicincome($type) {
        if (is_logged_in()) {
            ...
            $response['user_incomes'] = $this->User_model->get_records('tbl_repurchase_income', array('user_id' => $this->session->userdata['franchise_id'], 'type' => $type), 'id,user_id,amount,type,description,created_at');
            $this->load->view('incomes', $response);
        } else {
            redirect('Dashboard/User/login');
        }
    ...
    public function Magicincome_ledgar() {
        if (is_logged_in()) {
            ...
            $response['total_income'] = $this->User_model->get_single_record('tbl_repurchase_income', array('user_id' => $this->session->userdata['franchise_id']), 'ifnull(sum(amount),0) as total_income');
            $response['user_incomes'] = $this->User_model->get_records('tbl_repurchase_income', array('user_id' => $this->session->userdata['franchise_id']), 'id,user_id,amount,type,description,created_at');
            $this->load->view('incomes', $response);
        } else {
            redirect('Dashboard/User/login');
        }
    ...
    public function income_ledgar() {
        if (is_logged_in()) {
            ...
            $response['total_income'] = $this->User_model->get_single_record('tbl_income_wallet', array('user_id' => $this->session->userdata['franchise_id']), 'ifnull(sum(amount),0) as total_income');
            $response['user_incomes'] = $this->User_model->get_records('tbl_income_wallet', array('user_id' => $this->session->userdata['franchise_id']), 'id,user_id,amount,type,description,created_at');
            $this->load->view('incomes', $response);
        } else {
            redirect('Dashboard/User/login');
        }
    ...
    public function purchase_history() {
        if (is_logged_in()) {
            ...
            $response['orders'] = $this->User_model->get_records('tbl_orders', array('user_id' => $this->session->userdata['franchise_id']), '*');
            $i = 0;
    ...
    public function GenelogyTree($user_id = '') {
        if (is_logged_in()) {
            ...
            if(!empty($user)){
                if ($user_id == $this->session->userdata['franchise_id']) {
                    $validate_user = 1;
                } else {
                    $down_user = $this->User_model->get_single_record('tbl_downline_count', array('user_id' => $this->session->userdata['franchise_id'], 'downline_id' => $user_id), '*');
                    if (!empty($down_user)) {
                        $validate_user = 1;
                    }
                }
    ...
    public function Genelogy() {
        if (is_logged_in()) {
            $response = array();
            $response['directs'] = $this->User_model->get_records('tbl_users', array('sponser_id' => $this->session->userdata['franchise_id']), 'id,user_id,sponser_id');
            $response['directs'] = $this->User_model->get_records('tbl_users', array('sponser_id' => $this->session->userdata['franchise_id']), 'id,user_id,sponser_id');
    ...
    public function image_upload() {
        if (is_logged_in()) {
            ...
            $updres = $this->User_model->update('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), array('image' => $imageName));
