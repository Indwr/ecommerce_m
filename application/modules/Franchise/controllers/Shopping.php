...
class Shopping extends CI_Controller {
    ...
    public function index() {
        if (is_logged_in()) {
            $response = array();
            $response['products'] = $this->Shopping_model->get_product($this->session->userdata['franchise_id']);
            $this->load->view('Shopping/my_shop', $response);
        } else {
            redirect('Franshise/login');
        }
    }
    public function product_list() {
        if (is_logged_in()) {
            $response = array();
            $Mainuser = $this->Shopping_model->get_single_record('tbl_users',['user_id' => $this->session->userdata['franchise_id']],'id,user_id,role');
            //if($Mainuser['role'] == 'F'){
    ...
    public function checkStock(){
        if (is_logged_in()) {
            ...
            $check = $this->User_model->get_single_record('tbl_franchise_products',['user_id' => $this->session->userdata['franchise_id'],'product_id' => $product],'quantity');
            if($check['quantity'] < $qnty){
                $res['msg'] = 'Stock is not available';
            }
    ...
    public function order_list() {
        if (is_logged_in()) {
            $response = array();
            $response['orders'] = $this->Shopping_model->get_records('tbl_orders', array('user_id' => $this->session->userdata['franchise_id']), '*');
    ...
    public function Cart() {
        if (is_logged_in()) {
            $response = array();
            $response['cart_item'] = $this->Shopping_model->cart_items($this->session->userdata['franchise_id']);
        ...
        if (is_logged_in()) {
            ...
            if(!empty($user)){
                ...
                if(!empty($data['product_ids'])){
                    ...
                    $params['franchise_id'] = $this->session->userdata['franchise_id'];
                    ...
                    //if($data['coupans'] > $capable_coupans['coupans']){
                        //$params['coupan_discount'] =  $data['coupans'] * 50;
                        //$wallet_deduction = $params['amount'] - $params['coupan_discount'];
                        //$wallet_balance = $this->Shopping_model->get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['franchise_id']), 'ifnull(sum(amount),0) as wallet_balance');
                        ...
                        //if($wallet_balance['wallet_balance'] >= $wallet_deduction){
                            ...
                            if ($res == true) {
                                ...
                                foreach($response['products'] as $key => $product){
                                    ...
                                    $oldQuantity = $this->Shopping_model->get_single_record('tbl_franchise_products',['user_id' => $this->session->userdata['franchise_id'],'product_id' => $orderDetail['product_id']],'quantity');
                                    $this->Shopping_model->update('tbl_franchise_products',['user_id' => $this->session->userdata['franchise_id'],'product_id' => $orderDetail['product_id']],['quantity' => ($oldQuantity['quantity'] - $orderDetail['quantity'])]);
                                    $totalBV += $product['bv']; 
                                }

                                $this->repurchaseIncome($this->session->userdata['franchise_id'],$this->session->userdata['franchise_id'],$totalBV);
    ...
    public function add_to_cart($product_id) {
        if (is_logged_in()) {
            $response = array();
            $response['success'] = 0;
            $cart = $this->Shopping_model->get_single_record('tbl_cart', array('user_id' => $this->session->userdata['franchise_id'], 'product_id' => $product_id), '*');
            if (!empty($cart)) {
                $cartArr = array(
                    'quantity' => $cart['quantity'] + 1,
                );
                $res = $this->Shopping_model->update('tbl_cart', array('user_id' => $this->session->userdata['franchise_id'], 'product_id' => $product_id), $cartArr);
                if ($res == true) {
                    $response['success'] = 1;
        ...
        if (is_logged_in()) {
            ...
            if (!empty($cart)) {
                if ($cart['quantity'] > 1) {
                    ...
                    $res = $this->Shopping_model->delete('tbl_cart', array('user_id' => $this->session->userdata['franchise_id'], 'product_id' => $product_id));
                    if ($res == true) {
                        $response['success'] = 1;
    ...
    //         $response['tab'] = $tab;
    //         $response['success'] = 0;
    //         $response['shipping_address'] = (object) $this->Shopping_model->get_single_record('tbl_address', array('user_id' => $this->session->userdata['franchise_id'], 'type' => 'shipping'), '*');
    //         $response['tax'] = (object) $this->Shopping_model->get_single_record('tbl_tax', array('id' => 1), '*');
    //         $response['e_wallet'] = $this->Shopping_model->get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['franchise_id']), 'ifnull(sum(amount),0) as sum');
    //         $response['income_wallet'] = $this->Shopping_model->get_single_record('tbl_income_wallet', array('user_id' => $this->session->userdata['franchise_id']), 'ifnull(sum(amount),0) as sum');
    ...
    public function checkout() {
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'GET') {
                ...
                $response['shipping_charges'] = $this->Shopping_model->get_single_record('tbl_charges', array('type' => 'shipping_charges'), 'amount');
                $response['wallet_balance'] = $this->Shopping_model->get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['franchise_id']), 'ifnull(sum(amount),0) as wallet_balance');
    ...
    public function PaidInvoice() {
        if (is_logged_in()) {
            ...
            // $response['shipping_details'] = $this->Shopping_model->get_single_record('tbl_address', array('id' => $response['order']['address_id']), '*');
            // $response['order_details'] = $order_details;
            $response['user'] = $this->Shopping_model->get_single_record('tbl_users', array('user_id' => $this->session->userdata['franchise_id']), 'id,user_id,name,first_name,last_name,phone,email,sponser_id,address,country,state,city,postal_code,created_at');
...
//         $cart_items = $this->Shopping_model->cart_items($this->session->userdata['franchise_id']);
//         $cart_amount = 0;
//         $total_bv = 0;
...
//         echo'<input type="text" name="user_id" value="' . $this->session->userdata['franchise_id'] . '">';
