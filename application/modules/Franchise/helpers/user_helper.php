...
//    protected $CI;
    ...
    function is_logged_in() {
        ...
        if (isset($ci->session->userdata['franchise_id'])) {
            return true;
        } else {
            return false;
        }
...
if (!function_exists('userinfo')) {
    ...
    function userinfo() {
        ...
        $userdetails = $ci->user_model->get_single_object('tbl_users', array('user_id' => $ci->session->userdata['franchise_id']), '*');
        return $userdetails;
    }
...
if (!function_exists('pool_count')) {
    ...
    function pool_count() {
        $ci = & get_instance();
        $ci->load->model('user_model');
        $pool_count = $ci->user_model->get_single_object('tbl_pool', array('user_id' => $ci->session->userdata['franchise_id']), 'ifnull(count(id),0) as pool_count');
        return $pool_count;
    }
...
if (!function_exists('cart_items')) {
    ...
    function cart_items() {
        ...
        $userdetails = $ci->Shopping_model->cart_items($ci->session->userdata['franchise_id']);
        return $userdetails;
    }
