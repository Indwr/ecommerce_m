...
class Support extends CI_Controller {
    ...
    public function inbox() {
        if (is_admin()) {
            ...
            foreach($response['messages'] as $key => $user){
                $response['messages'][$key]['userinfo'] = $this->Main_model->get_single_record('tbl_users',['user_id' => $user['user_id']],'name');
            }
    ...
    public function contactUs() {
        if (is_admin()) {
            $response = array();
            $response['header'] = 'Contact Us Query';
            $response['messages'] = $this->Main_model->get_records('tbl_contactUs', array(), '*');
