...
//     }
    ...
    public function clubIncome(){
        if(date('D') == "Sat"){
            ...
            if(!empty($clubUser)){
                $weeklyBusiness = $this->Main_model->get_single_record('tbl_users',"week(topup_date) = week(now())",'ifnull(sum(package_amount),0) as business');
                $distribution = $weeklyBusiness['business']*0.05;
                $amount = $distribution/count($clubUser);
                foreach($clubUser as $key3 => $cu){
                    $userData = [
                        'user_id'  => $cu['user_id'],
                        'amount' => $amount,
                        'type' => 'club_income',
    ...
    public function royaltyIncome(){
        ...
        if(!empty($silverRoyalty)){
            $weeklyBusiness = $this->Main_model->get_single_record('tbl_users',"date(topup_date) = date(now())",'ifnull(sum(package_amount),0) as business');
            $distribution = $weeklyBusiness['business']*0.02;
            $amount1 = $distribution/count($silverRoyalty);
            foreach($silverRoyalty as $key12 => $sr){
                $userData1 = [
                    'user_id'  => $sr['user_id'],
                    'amount' => $amount1,
                    'type' => 'silver_income',
                    'description' => 'Star Royalty Income',
                ];
        ...
        if(!empty($goldRoyalty)){
            $weeklyBusiness = $this->Main_model->get_single_record('tbl_users',"date(topup_date) = date(now())",'ifnull(sum(package_amount),0) as business');
            $distribution = $weeklyBusiness['business']*0.02;
            $amount2 = $distribution/count($goldRoyalty);
            foreach($goldRoyalty as $key21 => $gr){
                $userData2 = [
                    'user_id'  => $gr['user_id'],
                    'amount' => $amount2,
                    'type' => 'gold_income',
                    'description' => 'Ruby Royalty Income',
                ];
        ...
        if(!empty($platinumRoyalty)){
            $weeklyBusiness = $this->Main_model->get_single_record('tbl_users',"date(topup_date) = date(now())",'ifnull(sum(package_amount),0) as business');
            $distribution = $weeklyBusiness['business']*0.03;
            $amount3 = $distribution/count($platinumRoyalty);
            foreach($platinumRoyalty as $key31 => $plr){
                $userData3 = [
                    'user_id'  => $plr['user_id'],
                    'amount' => $amount3,
                    'type' => 'platinum_income',
                    'description' => 'Royal Royalty Income',
                ];
    ...
    public function lifeTimeRoyalty(){
        $users = $this->Main_model->get_records('tbl_users',['leftPower >=' => '100','rightPower >=' => '100'],'user_id');
        $dailyBusiness = $this->Main_model->get_single_record('tbl_users',"date(topup_date) = date(now())",'ifnull(sum(package_amount),0) as business');
        $distribution = $dailyBusiness['business']*0.02;
        if(!empty($users)){
            $amount = $distribution/count($users);
            foreach($users as $key => $user){
                $userData = [
                    'user_id' => $user['user_id'],
                    'amount' => $amount,
                    'type' => 'life_time_royalty',
                    'description' => 'First Life Time Royalty',
                ];
                $this->Main_model->add('tbl_income_wallet',$userData);
            }
        }
    ...
    public function lifeTimeRoyalty2(){
        $users = $this->Main_model->get_records('tbl_users',['leftPower >=' => '600','rightPower >=' => '600'],'user_id');
        $dailyBusiness = $this->Main_model->get_single_record('tbl_users',"date(topup_date) = date(now())",'ifnull(sum(package_amount),0) as business');
        $distribution = $dailyBusiness['business']*0.03;
        if(!empty($users)){
            $amount = $distribution/count($users);
            foreach($users as $key => $user){
                $userData = [
                    ...
                    'amount' => $amount,
                    'type' => 'life_time_royalty',
                    'description' => 'Second Life Time Royalty',
                ];
                $this->Main_model->add('tbl_income_wallet',$userData);
            }
        }
    ...
    public function WithdrawCron(){
        ...
        //if($date == '11' || $date == '21' || $date == '30' ){
            ...
            // if(empty($checkCron)){
                ...
                pr($users);
                foreach($users as $key => $user){
                    if($user['total_amount'] % 10 == 0){
                        $amount = $user['total_amount'];
                    }else{
                        $deduct = $user['total_amount'] % 10;
